...
import {
  ...
  Clock, TestTube2, SlidersHorizontal,
  CalendarDays, IndianRupee, FlaskConical,
  Loader2, Download, ExternalLink, User,
} from "lucide-react";
...
const BOOKING_STATUS_CONFIG = {
  ...
  COMPLETED: { label: "Completed", color: "#059669", bg: "#f0fdf4", border: "#bbf7d0", dot: "#10b981" },
              ...
              <div className="px-5 pt-4 pb-0 border-b border-gray-100 flex-shrink-0">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-gray-400 leading-none mt-0.5">
                        {sidebarTab === "labs"
                          ? isLoadingLabs || isSearching ? "Fetching…" : `${nearbyLabs.length} near you`
                          : isLoadingBookings ? "Fetching…" : `${recentBookings.length} recent`
                        }
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    {sidebarTab === "labs" && (
                      <button className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center hover:bg-gray-200 transition-colors" title="Filter">
                ...
                <div className="flex -mb-px">
                  ...
                  <button
                    onClick={() => { setSidebarTab("bookings"); if (recentBookings.length === 0 && !isLoadingBookings) fetchRecentBookings(); }}
                ...
                <AnimatePresence mode="wait">
                  ...
                  {sidebarTab === "labs" && (
                    <motion.div
                      ...
                      className="space-y-2"
                    >
                      {(isLoadingLabs || isSearching) && [1, 2, 3, 4].map((i) => (
                      ...
                      {!isLoadingLabs && !isSearching && locationError && (
                        <div className="flex flex-col items-center justify-center py-16 text-center">
                          ...
                          <button onClick={getUserLocation} className="flex items-center gap-2 bg-emerald-500 text-white text-sm font-semibold px-4 py-2 rounded-xl shadow-sm hover:bg-emerald-600 transition-colors">
                      ...
                      {!isLoadingLabs && !isSearching && !locationError && nearbyLabs.map((lab, idx) => (
                        <motion.div
                          key={lab.id}
                            ...
                            <div className="flex items-center gap-3 mb-3">
                              {lab.contactNumber && (
                                <div className="flex items-center gap-1.5 text-xs text-gray-500">
                                  <Phone className="w-3 h-3" /><span>{lab.contactNumber}</span>
                                </div>
                              )}
                              {lab.slotCapacityOnline > 0 && (
                                <div className="flex items-center gap-1.5 text-xs text-emerald-600 font-medium">
                                  <Clock className="w-3 h-3" /><span>{lab.slotCapacityOnline} slots open</span>
                                </div>
                              )}
                            </div>
                    ...
                    >
                      ...
                      {!isLoadingBookings && !bookingsError && recentBookings.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-16 text-center">
                          ...
                          <button onClick={() => setSidebarTab("labs")} className="text-xs font-bold text-emerald-600 hover:text-emerald-700 flex items-center gap-1">
                            ...
                            <div className="flex min-w-0">
                              ...
                              <div className="flex-1 min-w-0 p-4">
                                ...
                                <div className="flex items-center gap-3 mb-2.5">
                                  ...
                                  {totalPrice > 0 && (
                                    <div className="flex items-center gap-0.5 text-xs font-black text-gray-800 ml-auto">
                                      <IndianRupee className="w-3 h-3" />{totalPrice}
                                    </div>
                                  )}
                                </div>
                                ...
                                <div className="pt-2.5 border-t border-gray-50 flex items-center justify-between">
                                  ...
                                  {booking.lab?.id && (
                                    <Link to={`/lab/${booking.lab.id}`} className="text-[11px] font-bold text-emerald-600 hover:text-emerald-700 flex items-center gap-1 transition-colors">
...
function BookingReportLinks({ bookingTests }) {
  ...
  useEffect(() => {
    ...
    bookingTests.forEach((bt) => {
      ...
      api.get(`/booking/${bt.id}/reports`)
        ...
        .finally(() => setLoadingIds((prev) => { const n = new Set(prev); n.delete(bt.id); return n; }));
    });
  ...
  if (allReports.length === 0) return (
    <div className="text-[11px] text-gray-400 italic mb-2.5">No reports yet</div>
  );
